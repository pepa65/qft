...
fn sender<F: Fn(f32)>(cli: &Cli, on_progress: F) {
    ...
    let buf = buf.leak();
    let mut file = File::open(cli.file.as_str())
    ...
    sc.write_safe(&len.to_be_bytes(), WRITE_DELAY)
    ...
    loop {
        ...
        if (bytes_sent % (cli.bitrate * KB_FAC) as u64) < cli.bitrate as u64 {
            ...
            print!(
                ...
                bytes_sent,
                cli.bitrate as u64 * KB_FAC as u64 / elapsed
            );
            stdout().flush().unwrap();
            time = unix_millis();
        }
    ...
    if env::var("QFT_USE_TIMED_HOLEPUNCH").is_ok() {
        ...
        while !stop {
            thread::sleep(Duration::from_millis(
                PUNCH_DELAY - (unix_millis() % PUNCH_DELAY),
            ));
